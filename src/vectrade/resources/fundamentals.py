...
class Fundamentals:
    ...
    def income_statement(self, symbol: str, *, period: str = "annual") -> list[IncomeStatement]:
        ...
        response = self._http.get(f"/vq/fundamentals/{encode_path_param(symbol)}/income", params={"period": period})
        response.raise_for_status()
    ...
    def balance_sheet(self, symbol: str, *, period: str = "annual") -> list[BalanceSheet]:
        ...
        response = self._http.get(f"/vq/fundamentals/{encode_path_param(symbol)}/balance-sheet", params={"period": period})
        response.raise_for_status()
...
class AsyncFundamentals:
    ...
    async def income_statement(self, symbol: str, *, period: str = "annual") -> list[IncomeStatement]:
        response = await self._http.get(f"/vq/fundamentals/{encode_path_param(symbol)}/income", params={"period": period})
        response.raise_for_status()
        return [IncomeStatement.model_validate(item) for item in response.json()["data"]]

    async def balance_sheet(self, symbol: str, *, period: str = "annual") -> list[BalanceSheet]:
        response = await self._http.get(f"/vq/fundamentals/{encode_path_param(symbol)}/balance-sheet", params={"period": period})
        response.raise_for_status()
        return [BalanceSheet.model_validate(item) for item in response.json()["data"]]
